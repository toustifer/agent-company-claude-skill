...
```
.mycompany/
  config.json              ← 语言配置，Git 跟踪
  identity.json            ← 用户身份（userId、name），Git 忽略
  leader/
    leader.json            ← DAG + Workers 状态
    playbook.md            ← Leader 工作标准
    diary/                 ← Leader 工作日志
  workers/
    <worker-id>/
      session.json         ← Worker 执行记录 + retrospective
      experience.md        ← Worker 业务经验（业务+技术栈+坑位+演化）
      diary/               ← Worker 工作日志
  memory/
    project.md             ← 项目技术事实
    decisions.md           ← 架构决策
  playbook/
    worker.md              ← 跨 Worker 通用打法
    reviewer.md            ← Reviewer 审查标准
  dag.html                 ← 可视化面板
  tasks/
    inbox.md               ← 待处理
    completed.md           ← 已完成 + Review
```

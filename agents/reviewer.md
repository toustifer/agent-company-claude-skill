...
- Criteria Check:
  ...
  2. [PASS] token 持久化到 storage — src/stores/user.js 第 88 行
...
- **ALL criteria pass** → Update leader.json: task status → `completed`, add review summary
- **Any criterion fails** → Update leader.json: task status → `pending`, add feedback notes, re-dispatch Worker
